...
private extension VideoViewLayout {
    //全屏布局
    func layoutFullSessionView(view: UIView, inContainerView containerView: UIView) -> [NSLayoutConstraint] {
    ...
    //右上角布局
    func layoutCornerSessionView(view: UIView, inContainerView containerView: UIView) -> [NSLayoutConstraint] {
        containerView.addSubview(view)
        var layouts = [NSLayoutConstraint]()
    ...
    func layoutSmallSessionViews(smallViews: [UIView], inContainerView containerView: UIView) -> [NSLayoutConstraint] {
        let ratio: CGFloat
        ...
        if isTopLeft {
            ...
            let scrollConstraintsV = NSLayoutConstraint.constraintsWithVisualFormat("V:|[view]-(\(scrollBottomSpace))-|", options: [], metrics: nil, views: ["view": scrollView])
            ...
            layouts.appendContentsOf(scrollConstraintsV)
            layouts.appendContentsOf([scrollWidth, scrollRight])
        }
    ...
    //等分屏布局
    func layoutEqualSessionViews(allViews: [UIView], inContainerView containerView: UIView) -> [NSLayoutConstraint] {
